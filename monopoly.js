function Game() {
	...
	this.eliminatePlayer = function() {
		...
		if (pcount === 1) {
			...
			popup("<p>Parabens, " + player[1].name + ", você venceu o jogo.</p><div>");
	...
	this.bankruptcyUnmortgage = function() {
		...
		var HTML = "<p>" + player[p.creditor].name + ", você pode quitar qualquer uma das seguintes propriedades, sem juros, clicando nelas. Clique em OK quando terminar.</p><table>";
		var price;
		...
		for (var i = 0; i < 40; i++) {
			...
			if (sq.owner == p.index && sq.mortgage) {
				...
				//Gambiarra
				HTML += "' onmouseover='showdeed(" + i + ");' onmouseout='hidedeed();'></td><td class='propertycellname'><a href='javascript:void(0);' title='Desipotecar " + sq.name + " por $" + price + ".' onclick='if (" + price + " <= player[" + p.creditor + "].money) {player[" + p.creditor + "].pay(" + price + ", 0); square[" + i + "].mortgage = false; addAlert(\"" + player[p.creditor].name + " não hipotecado " + sq.name + " por $" + price + ".\");} this.parentElement.parentElement.style.display = \"none\";'>Unmortgage " + sq.name + " ($" + price + ")</a></td></tr>";
	...
	this.resign = function() {
		popup("<p>Você tem certeza que quer desistir?</p>", game.bankruptcy, "Sim/Não");
	};
	...
	this.bankruptcy = function() {
		...
		if (pcount === 2 || bankruptcyUnmortgageFee === 0 || p.creditor === 0) {
			...
			addAlert(pcredit.name + " pagou $" + bankruptcyUnmortgageFee + " juros sobre as propriedades hipotecadas recebidas de " + p.name + ".");
...
function updateOwned() {
	...
	if (p.communityChestJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox40' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Sair da Prisão Carta Grátis</td></tr>";
	...
	if (p.chanceJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox41' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Sair da Prisão Carta Grátis</td></tr>";
	}
...
function updateOption() {
	...
	if (sq.mortgage) {
		document.getElementById("mortgagebutton").value = "Desipotecar ($" + Math.round(sq.price * 0.55) + ")";
		document.getElementById("mortgagebutton").title = "Desipotecar " + sq.name + " for $" + Math.round(sq.price * 0.55) + ".";
		$("#buyhousebutton").hide();
		$("#sellhousebutton").hide();
		...
		if (sq.groupNumber >= 3) {
			...
			buyhousebutton.title = "Comprar a casa para $" + sq.houseprice;
			sellhousebutton.title = "Vender a casa para $" + (sq.houseprice * 0.5);

			if (sq.house == 4) {
				buyhousebutton.value = "Comprar hotel ($" + sq.houseprice + ")";
				buyhousebutton.title = "Comprar hotel para $" + sq.houseprice;
			}
			if (sq.hotel == 1) {
				$("#buyhousebutton").hide();
				sellhousebutton.value = "Vender hotel ($" + (sq.houseprice * 0.5) + ")";
				sellhousebutton.title = "Vender hotel para $" + (sq.houseprice * 0.5);
			}
			...
			for (var i = 0; i < max; i++) {
				...
				if (s.owner !== sq.owner) {
					...
					buyhousebutton.title = "Antes de poder comprar uma casa, você deve possuir todas as propriedades deste grupo de cores.";
				} else {
			...
			if (!allGroupUnmortgaged) {
				buyhousebutton.disabled = true;
				buyhousebutton.title = "Antes de poder comprar uma casa, você deve quitar todas as propriedades deste grupo de cores.";
			}
			...
			if (sq.house > minhouse) {
				...
				if (sq.house == 1) {
					buyhousebutton.title = "Antes de poder comprar outra casa, todas as outras propriedades deste grupo de cores devem ter uma casa.";
				} else if (sq.house == 4) {
					buyhousebutton.title = "Antes de poder comprar um hotel, todas as outras propriedades deste grupo de cores devem ter 4 casas.";
				} else {
					buyhousebutton.title = "Antes de poder comprar uma casa, todas as outras propriedades deste grupo de cores devem ter " + sq.house + " casas.";
				}
			}
			if (sq.house < maxhouse) {
				...
				if (sq.house == 1) {
					sellhousebutton.title = "Antes de poder vender a casa, todas as outras propriedades deste grupo de cores devem ter uma casa.";
				} else {
					sellhousebutton.title = "Antes de poder vender uma casa, todas as outras propriedades deste grupo de cores devem ter " + sq.house + " casas.";
				}
			}
			...
			if (!allGroupUninproved) {
				document.getElementById("mortgagebutton").title = "Antes que uma propriedade possa ser hipotecada, todas as propriedades de seu grupo de cor devem ter o nivel diminuido.";
...
function gotojail() {
	...
	document.getElementById("nextbutton").title = "Terminar turno e avançar para o próximo jogador.";
...
function advance(destination, pass) {
	...
	if (typeof pass === "number") {
		if (p.position < pass) {
			...
			p.position = pass;
			p.money += 200;
			addAlert(p.name + " coletou $200 de salário por passar no início.");
		}
	}
...
function payfifty() {
	...
	addAlert(p.name + " pagou a multa de $50 para sair da prisão.");
	updateMoney();
	updatePosition();
}
...
function useJailCard() {
	...
	addAlert(p.name + " usou a carta\"Sair da Prisão Grátis\".");
	updateOwned();
	updatePosition();
}
...
function sellHouse(index) {
	...
	if (sq.hotel === 1) {
		...
		sq.house--;
		addAlert(p.name + " vendeu a casa em " + sq.name + ".");
	}
...
function showStats() {
	...
	for (var x = 1; x <= pcount; x++) {
		...
		if (p.communityChestJailCard) {
			...
			HTML += "<tr><td class='statscellcolor'></td><td class='statscellname'>Sair da Cadeia Grátis Carta</td></tr>";
...
function buy() {
	...
	if (p.money >= cost) {
		...
		property.owner = turn;
		updateMoney();
		addAlert(p.name + " comprou " + property.name + " para " + property.pricetext + ".");
		...
		popup("<p>" + p.name + ", você precisa de $" + (property.price - p.money) + " para comprar " + property.name + ".</p>");
	}
}
...
function mortgage(index) {
	...
	document.getElementById("mortgagebutton").value = "Desipoteque por $" + unmortgagePrice;
	document.getElementById("mortgagebutton").title = "Desipoteque " + sq.name + " por $" + unmortgagePrice + ".";
...
function unmortgage(index) {
	...
	sq.mortgage = false;
	document.getElementById("mortgagebutton").value = "Hipotecado por $" + mortgagePrice;
	document.getElementById("mortgagebutton").title = "Hipoteca " + sq.name + " por $" + mortgagePrice + ".";

	addAlert(p.name + " desipotecado " + sq.name + " por $" + unmortgagePrice + ".");
	updateOwned();
	return true;
}
...
function land(increasedRent) {
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			document.getElementById("landed").innerHTML = "<div>Você pousou no <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.<input type='button' onclick='buy();' value='Comprar ($" + s.price + ")' title='Comprar " + s.name + " for " + s.pricetext + ".'/></div>";
		}
	...
	if (s.owner !== 0 && s.owner != turn && !s.mortgage) {
		...
		document.getElementById("landed").innerHTML = "Você pousou no " + s.name + ". " + player[s.owner].name + " coletou $" + rent + " de aluguel.";
	} else if (s.owner > 0 && s.owner != turn && s.mortgage) {
		document.getElementById("landed").innerHTML = "Você pousou no " + s.name + ". A propriedade está hipotecada; nenhum aluguel foi cobrado.";
	}
	...
	if (p.position === 30) {
		...
		if (p.human) {
			popup("<div>Vá para a cadeia. Vá diretamente para a prisão. Não passe Início. Não colete $ 200.</div>", gotojail);
		} else {
			gotojail();
		}
...
function roll() {
	...
	if (die1 == die2) {
		addAlert(p.name + " tirou " + (die1 + die2) + " - doubles.");
	} else {
	...
	if (die1 == die2 && !p.jail) {
		...
		if (doublecount < 3) {
			document.getElementById("nextbutton").value = "Role novamente";
			document.getElementById("nextbutton").title = "Você tirou doubles. Role novamente.";

		} else if (doublecount === 3) {
			p.jail = true;
			doublecount = 0;
			addAlert(p.name + " tirou doubles três vezes seguidas.");
			updateMoney();


			if (p.human) {
				popup("Você tirou doubles três vezes seguidas. Vá para a prisão.", gotojail);
			} else {
				gotojail();
			}
	...
	if (p.jail === true) {
		...
		if (die1 == die2) {
			...
			addAlert(p.name + " tirou doubles para sair da cadeia.");
			...
			if (p.jailroll === 3) {
				...
				if (p.human) {
					popup("<p>Você deve pagar a multa de $50.</p>", function() {
						payfifty();
		...
		if (p.position >= 40) {
			p.position -= 40;
			p.money += 200;
			addAlert(p.name + " coletou o salário de $200 ao passar pelo Início.");
		}
...
function play() {
	...
	document.getElementById("nextbutton").value = "Rodar o Dado";
	document.getElementById("nextbutton").title = "Rode o dado e mova sua peça de acordo.";
	...
	if (p.jail) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "Você está na cadeia.<input type='button' title='Pague a multa de $50 para sair da cadeia imediatamente.' value='Pague a multa de $50' onclick='payfifty();' />";

		if (p.communityChestJailCard || p.chanceJailCard) {
			document.getElementById("landed").innerHTML += "<input type='button' id='gojfbutton' title='Use &quot;Sair da Cadeia de Graça&quot; carta.' onclick='useJailCard();' value='Usa carta' />";
		}

		document.getElementById("nextbutton").title = "Rode o dado. Se você tirar doubles, você irá sair da cadeia.";

		if (p.jailroll === 0)
			addAlert("Esse é o primeiro turno de " + p.name + " na cadeia.");
		else if (p.jailroll === 1)
			addAlert("Esse é o segundo turno de " + p.name + " na cadeia.");
		else if (p.jailroll === 2) {
			document.getElementById("landed").innerHTML += "<div>NOTA: Se você não tirar doubles neste turno você <i>DEVE</i> pagar a multa de $50.</div>";
			addAlert("Esse é o terceiro turno de " + p.name + " na cadeia.");
		}
...
window.onload = function() {
	...
	$("#mortgagebutton").click(function() {
		...
		if (s.mortgage) {
			if (player[s.owner].money < Math.round(s.price * 0.55)) {
				popup("<p>Você precisa de mais $" + (Math.round(s.price * 0.55) - player[s.owner].money) + " para desipotecar " + s.name + ".</p>");

			} else {
				popup("<p>" + player[s.owner].name + ", você tem certeza que quer desipotecar " + s.name + " para $" + Math.round(s.price * 0.55) + "?</p>", function() {
					unmortgage(checkedProperty);
				}, "Sim/Nao");
			}
		} else {
			popup("<p>" + player[s.owner].name + ", você tem certeza que quer hipotecar " + s.name + " para $" + Math.round(s.price * 0.5) + "?</p>", function() {
	...
	$("#buyhousebutton").on("click", function() {
		...
		if (p.money < s.houseprice) {
			if (s.house === 4) {
				popup("<p>Você precisa de mais $" + (s.houseprice - player[s.owner].money) + " para comprar um hotel para " + s.name + ".</p>");
				return;
			} else {
				popup("<p>Você precisa de mais $" + (s.houseprice - player[s.owner].money) + " para comprar um hotel para " + s.name + ".</p>");
				return;
		...
		if (s.house < 4 && houseSum >= 32) {
			popup("<p>Todas as 32 casas estão hipotecadas. Você deve esperar até que uma esteja disponível.</p>");
			return;
		} else if (s.house === 4 && hotelSum >= 12) {
			popup("<p>Todos os 12 hotéis estão hipotecados. Você deve esperar até que um esteja disponível.</p>");
			return;
		}
